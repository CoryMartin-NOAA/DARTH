...
dir_modules=$HOMEDARTH/modulefiles
...
if [ $target = wcoss_d ]; then
    ...
    module load modulefile.DARTH.$target
elif [ $target = wcoss -o $target = gaea ]; then
    module purge
    module load $dir_modules/modulefile.DARTH.$target
elif [ $target = hera -o $target = cheyenne -o $target = orion ]; then
    module purge
    source $dir_modules/modulefile.DARTH.$target
